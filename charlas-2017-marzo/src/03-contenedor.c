...
int main(int argc, char *argv[]){
   ...
   * como flag la creación de nuevos namespace de file system, UTS, PID, IPC y
   * usuarios.
   */
	printf("[*] Creando un nuevo proceso hijo!\n");
	pid = clone(proceso_hijo, hijo_stack + STACK_SIZE, CLONE_NEWNS | CLONE_NEWUTS | CLONE_NEWPID | CLONE_NEWIPC | CLONE_NEWUSER | SIGCHLD, &args);
  if (pid == -1){
